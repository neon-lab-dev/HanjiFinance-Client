...
const PaymentProductsCards = ({ items } : any) => {
 ...
 const handleDecrement = (productId: string, sizeId: string, currentQuantity: number) => {
    dispatch(updateQuantity({
        productId,
        sizeId,
        quantity: currentQuantity - 1,}))}
        ...
        >
          ...
          <div className="flex gap-2 h-fit">
           <div className="gap-3 py-2 px-3 text-neutral-115 flex items-center justify-center bg-secondary-25 rounded-lg">
            <div onClick={()=>handleDecrement(item.productId, item.sizeId,item.quantity)} className="cursor-pointer px-1">-</div>
                {item.quantity}
                 <div onClick={()=>handleIncrement(item.productId, item.sizeId,item.quantity)} className="cursor-pointer px-1">+</div>
             </div>
